...
if (matches[1]:lower() == "check".."li".."cen".."se" or matches[1] == "چک لایسنس") and is_botOwner(msg) then
	url = http.request(h0i..""..h0ii..""..h0iii..""..h0iiii..''..private..'l'..'i'..'c'..'e'..'n'..'s'..'e:'..Mojavez_A_Plus..''..JFormat)
	data = json:decode(url)
	if data.status == ac or data.status == noac then
		status = "فعال"
	elseif data.status == bl then
		status = "مسدود شده"
	end
	return "کد لایسنس: "..Mojavez_A_Plus.."\nوضعیت لایسنس: "..status.."\nربات متصل به لایسنس: "..data.bot
end
--[[ banhammer ]]
...
if not redis:get(string.reverse(":esneciLevitcA")..""..Mojavez_A_Plus) then
	url = http.request(h0i..""..h0ii..""..h0iii..""..h0iiii..''..private..'l'..'i'..'c'..'e'..'n'..'s'..'e:'..Mojavez_A_Plus..''..JFormat)
	data = json:decode(url)
	
	if data.status == ac then
		if data.checkip == "yes" then
			ip_ = io.popen("ip addr show"):read('*all')
			ip = ip_:match("inet %d+.%d+.%d+.%d+/%d+ brd")
			ip = ip:gsub("inet ","")
			ip = ip:gsub("/%d+ brd","")
			if ip == data.ip then
				redis:set(string.reverse(":esneciLevitcA")..""..Mojavez_A_Plus, true)
		...
		elseif data.checkip == "no" and data.bot == bot.id then
			redis:set(string.reverse(":esneciLevitcA")..""..Mojavez_A_Plus, true)
			text = "لایسنس با موفقیت فعال شد ☑️"
			tdcli.sendMessage(msg.chat_id_, 0, 1, text, 1, 'md')
		end
	elseif data.status == noac then
...
elseif redis:get(string.reverse(":esneciLevitcA")..""..Mojavez_A_Plus) and not redis:get("Check".."Li".."cen".."seDaily") then
	redis:setex("Check".."Li".."cen".."seDaily", 3600, true)
	url = http.request(h0i..""..h0ii..""..h0iii..""..h0iiii..''..private..'l'..'i'..'c'..'e'..'n'..'s'..'e:'..Mojavez_A_Plus..''..JFormat)
	data = json:decode(url)
	if data.status == bl then
		text = "لایسنس مسدود شده است!"
		tdcli.sendMessage(msg.chat_id_, 0, 1, text, 1, 'md')
		redis:del(string.reverse(":esneciLevitcA")..""..Mojavez_A_Plus)
	else
		if data.checkip == "yes" then
			ip_ = io.popen("ip addr show"):read('*all')
			ip = ip_:match("inet %d+.%d+.%d+.%d+/%d+ brd")
